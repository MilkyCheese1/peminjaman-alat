...
const loadMessages = () => {
  ...
  messages.value = ChatService.getMessages(userInfo.value.id_user, 'customer')
  hasUnread.value = ChatService.hasUnreadMessages(userInfo.value.id_user, 'customer')
}
    ...
    <CardContent class="flex flex-col h-96 space-y-4">
      ...
      <div ref="messagesContainer" class="flex-1 overflow-y-auto space-y-3 bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
        ...
        >
          ...
          >
            <p>{{ msg.message }}</p>
            <p :class="['text-xs mt-1', msg.sender_type === 'customer' ? 'text-blue-100' : 'text-muted-foreground']">
      ...
      <div class="flex gap-2">
        <input
          ...
          :disabled="isSending"
          class="flex-1 px-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 bg-background disabled:opacity-50"
        />
        <button
          @click="sendMessage"
          :disabled="!messageInput.trim() || isSending"
          class="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
          <Send :size="16" />
        </button>
      </div>
